...
public class JuneNoiseIAQPlotter {
    ...
    public static void main(String[] args) {
        ...
        File[] files = folder.listFiles((dir, name) -> name.contains("Noise"));

        if (files == null || files.length == 0) {
            System.out.println("No noise files found.");
            return;
        }
        ...
        for (File file : files) {
            ...
            if (count > 0) {
                double avgNoise = dailySum / count;
                String iaqLevel = classifyNoise(avgNoise);
                dataset.addValue(mapIAQToNumeric(iaqLevel), "IAQ Grade", "Day " + day);
            }
        }

        JFreeChart chart = ChartFactory.createLineChart(
                "IAQ Grade Based on Daily Noise Levels",
                "Day of June",
                "IAQ Grade",
                dataset,
        ...
        JFrame frame = new JFrame("Noise IAQ Grade Chart");
    ...
    private static String classifyNoise(double noise) {
        if (noise <= 30) return "S";
        else if (noise <= 40) return "A";
        else if (noise <= 50) return "B";
        else if (noise <= 60) return "C";
        else if (noise <= 70) return "D";
        else return "F";
    }
